...
#'                \itemize{
#'                \item \code{higherOrderMarkovModel( ...) } is the costructor
...
#'    \item \code{order } an integer parameter (default is \code{2}). It is the order of the markov rule (e.g. 2 means next transition is influenced by actual AND previous state).
#'   }
#' @useDynLib pMineR    
#' @export 
#' @import plyr
...
#' EVENTName = "DES")
#' 
#' # now create an object higherOrderMarkovModel
#' obj.MM<-higherOrderMarkovModel();    
#' 
...
higherOrderMarkovModel<-function( parameters.list = list() ) {
  MMatrix<-''
  footPrint<-''
  model.grViz<-'';
  model.XML<-'';
  ...
  loadDataset<-function( dataList ) { 
    ...
    dat <- dataList$wordSequence.raw
    uniqueEvents <- unique(unlist(dataList$wordSequence.raw))
    for(i in 1:length(dat)){
      dat[[i]] <- append("BEGIN", dat[[i]])
      dat[[i]] <- append(dat[[i]],"END")
    }
    
    if(!is.null(parameters$considerAutoLoop)) order <- parameters$order 
    else order <- 2 
    aaa <- rbind.fill(lapply(dat,function(y){as.data.frame(t(y),stringsAsFactors=FALSE)}))
    transMatrix.tmp <- matrix(nrow = length(uniqueEvents) + 2 , ncol = length(uniqueEvents) + 1, data = 0)
      for (i in 1:order){
      transMatrix.tmp <- transMatrix.tmp + Markovmatrix(as.matrix(aaa),i)
      }
    
    transMatrix.tmp <- cbind(transMatrix.tmp,"BEGIN"=0)
    transMatrix.tmp <- transMatrix.tmp[ , order(colnames(transMatrix.tmp))]
    #moltiplico per tutti gli eventi
    MMatrix<<-transMatrix.tmp
    
    # calcola la matrice delle percentuali e quella delle percentuali senza i loop
    # MMatrix.perc
    transMatrix.tmp.perc <- transMatrix.tmp / rowSums(transMatrix.tmp)
    transMatrix.tmp.perc["END",] <- 0
    MMatrix.perc<<- transMatrix.tmp.perc
    
    # MMatrix.perc.noLoop
    diag(transMatrix.tmp.perc)<-0;

    MMatrix.perc.noLoop<<-transMatrix.tmp.perc     
  ...
  #===========================================================
  # Markovmatrix
  # Costruisce la matrice di transizione al grado l (default=2)
  #===========================================================
  
  Markovmatrix <- function(X,l){
    tt <- table(X[,-c((ncol(X)-l+1):ncol(X))] , c(X[,-c(1:l)]))
    #tt <- tt / rowSums(tt)
    missingCols <-  rownames(tt[which(!rownames(tt) %in% colnames(tt)),])
    missingCols <-  missingCols[which(!missingCols %in% c("BEGIN","END"))]
    tt["END",] <- 0
      for(name in missingCols){
        tt <- cbind(tt, 0)
        colnames(tt)[dim(tt)[2]] <- name
      }
    tt <- tt[ , order(colnames(tt))]
    return(tt)
  }
